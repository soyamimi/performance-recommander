...
class LightFMRecommender:
    ...
    def evaluate(self, k):
        precision = precision_at_k(
            self.model, self.interactions, item_features=self.item_features, k=k
        ).mean()
        auc = auc_score(
            self.model, self.interactions, item_features=self.item_features
        ).mean()
    ...
    def recommend(self, user_id):
        ...
        scores = self.model.predict(
            user_id, np.arange(n_items), item_features=self.item_features
        )
...
if __name__ == "__main__":
    ...
    recommender = LightFMRecommender(
        interactions, item_features=item_features, epochs=30, loss="warp", num_threads=2
    )
